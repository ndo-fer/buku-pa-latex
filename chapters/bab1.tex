...
Program Jaminan Kesehatan Nasional (JKN) merupakan sistem jaminan sosial kesehatan yang bertujuan memperluas akses masyarakat terhadap layanan kesehatan. Dalam penyelenggaraannya, BPJS Kesehatan mengelola pembiayaan layanan kesehatan melalui mekanisme klaim yang diajukan oleh fasilitas kesehatan. Pada fasilitas kesehatan rujukan tingkat lanjutan (FKRTL), pembayaran layanan dilakukan menggunakan sistem Indonesian Case-Based Groups (INA-CBG), yaitu sistem pembayaran berbasis kelompok kasus dalam penyelenggaraan JKN \citep{kemenkes2014,kemenkes2016}.

INA-CBG bekerja dengan mengelompokkan kasus pasien berdasarkan diagnosis, prosedur, karakteristik layanan, serta tingkat keparahan kasus. Sistem \textit{casemix} dalam INA-CBG mengelompokkan diagnosis dan prosedur yang memiliki kemiripan ciri klinis serta penggunaan sumber daya, kemudian menghasilkan kelompok kasus dan tarif klaim melalui perangkat lunak \textit{grouper} \citep{kemenkes2016}. Dengan mekanisme tersebut, pembayaran klaim tidak dihitung berdasarkan setiap tindakan secara terpisah, melainkan berdasarkan kelompok kasus dan tarif yang telah ditetapkan.

Skala layanan JKN yang besar membuat proses verifikasi klaim membutuhkan dukungan mekanisme prioritisasi berbasis data. Hingga 30 November 2025, pemanfaatan layanan kesehatan JKN telah mencapai 664,3 juta kunjungan dengan total beban jaminan sebesar Rp172,6 triliun, sehingga pemeriksaan klaim perlu diarahkan pada klaim yang memiliki indikasi risiko lebih tinggi \citep{djsn2025monthly}.

Ketergantungan nilai klaim terhadap diagnosis, prosedur, \textit{severity level}, dan hasil \textit{grouping} membuat ketepatan dokumentasi serta pengkodean menjadi aspek yang sangat penting. Kesalahan atau manipulasi pada kode diagnosis dan prosedur dapat memengaruhi kelompok kasus dan tarif yang dihasilkan. Dalam konteks ini, \textit{upcoding} dipahami sebagai perubahan atau penggunaan kode diagnosis maupun prosedur yang menghasilkan tarif lebih tinggi daripada kondisi yang seharusnya \citep{maryana2024}. Risiko tersebut juga berkaitan dengan struktur insentif sistem pembayaran berbasis kelompok kasus, karena perubahan tarif dapat memengaruhi perilaku pengkodean rumah sakit dan mendorong kecenderungan pengodean pada tingkat keparahan yang lebih tinggi \citep{chalkley2022,crespin2024}.

Dalam literatur asuransi kesehatan, \textit{fraud} dipahami sebagai tindakan penipuan, penyimpangan, atau misrepresentasi yang dilakukan untuk memperoleh manfaat yang tidak semestinya dari sistem pembiayaan kesehatan \citep{villegas2021,timofeyev2022}. Salah satu manifestasi \textit{fraud} oleh penyedia layanan adalah \textit{upcoding}, yaitu penggunaan kode klaim yang tidak sesuai untuk memperoleh nilai ekonomi yang lebih besar. Dalam konteks JKN, beberapa bentuk \textit{fraud} yang dilaporkan antara lain manipulasi prosedur medis, pemalsuan dokumen, penggunaan identitas fiktif, serta \textit{upcoding} \citep{nurlianti2025,muliarta2023}. Dengan demikian, \textit{upcoding} perlu dipahami sebagai bagian dari problem domain \textit{fraud} klaim kesehatan, meskipun penetapan \textit{fraud} aktual tetap memerlukan proses verifikasi dan audit lebih lanjut.

Dampak \textit{fraud} dan \textit{upcoding} tidak hanya terbatas pada peningkatan nilai klaim, tetapi juga dapat mengganggu efisiensi pembiayaan, kualitas data administratif, serta kepercayaan terhadap sistem kesehatan \citep{villegas2021,timofeyev2022}. Pada sisi data, ketidaktepatan kode diagnosis dan prosedur dapat memengaruhi kualitas data klaim yang digunakan untuk verifikasi, evaluasi, dan pengambilan keputusan. Rekam medis menjadi dasar penting dalam proses klaim dan dapat digunakan untuk menelusuri potensi \textit{fraud} dalam JKN \citep{sugiarti2021}. Selain itu, moral hazard pada penyedia layanan kesehatan dapat muncul dalam bentuk \textit{upcoding}, \textit{readmission}, dan \textit{unnecessary admission}, sehingga risiko \textit{fraud} perlu dilihat sebagai persoalan yang melibatkan interaksi antara aturan klaim, dokumentasi medis, pengkodean, dan perilaku aktor yang terlibat \citep{syafrawati2023}.

Dalam praktik pengendalian klaim, proses verifikasi berperan penting untuk menjaga kesesuaian klaim dengan kaidah koding, aspek klinis, dan administrasi. Panduan Manual Verifikasi Klaim INA-CBG menjelaskan bahwa perbedaan persepsi antara verifikator BPJS Kesehatan dan \textit{coder} fasilitas kesehatan masih menjadi salah satu permasalahan dalam penyelesaian klaim INA-CBG, sehingga verifikasi diperlukan untuk meminimalkan \textit{dispute claim} dari sisi koding, klinis, dan administrasi \citep{bpjs2018}. Namun, volume dan kompleksitas data klaim membuat proses pemeriksaan manual perlu didukung oleh pendekatan analitis yang mampu membantu memprioritaskan klaim berisiko.

Pendekatan berbasis data menjadi relevan karena \textit{fraud} klaim kesehatan memiliki pola yang kompleks, beragam, dan dapat berubah. Najar dkk. menekankan perlunya integrasi antara deteksi berbasis data, pencegahan, dan respons hukum dalam menangani \textit{medical fraud and abuse} \citep{najar2025}. Machine learning telah digunakan dalam berbagai penelitian untuk mendeteksi potensi \textit{fraud} pada klaim asuransi kesehatan karena mampu mempelajari pola dari data dan mengidentifikasi indikasi penyimpangan yang tidak selalu mudah ditemukan melalui aturan manual \citep{nabrawi2023,hamid2024,curtis2025}.

Meskipun demikian, penerapan machine learning pada data klaim kesehatan menghadapi tantangan penting, yaitu tidak selalu tersedianya label \textit{fraud} aktual pada level klaim. Dalam kondisi tersebut, pendekatan deteksi anomali, semi-supervised learning, dan pembentukan label awal dapat digunakan untuk membantu model mengenali klaim yang memiliki pola berbeda dari mayoritas klaim lain \citep{settipalli2023,yoon2022,bakker2025}. Shekhar dkk. menunjukkan bahwa deteksi \textit{fraud} layanan kesehatan dapat dilakukan menggunakan pendekatan \textit{unsupervised machine learning} yang tidak bergantung pada data berlabel dan tetap menyediakan penjelasan bagi pengguna \citep{shekhar2023}. Meulemeester dkk. juga menekankan pentingnya deteksi anomali yang dapat dijelaskan pada data asuransi kesehatan \citep{meulemeester2025}. Gagasan pseudo-labeling juga relevan karena memanfaatkan label sementara atau label proksi sebagai target pembelajaran ketika label aktual tidak tersedia secara memadai \citep{lee2013}.

Selain kemampuan mendeteksi indikasi risiko, model yang digunakan pada domain klaim kesehatan juga perlu dapat dijelaskan. Hasil pemodelan tidak seharusnya berhenti pada pemberian label atau skor risiko, karena auditor dan verifikator tetap membutuhkan alasan mengapa suatu klaim diprioritaskan untuk diperiksa. Model prediksi \textit{fraud} asuransi kesehatan perlu memperhatikan aspek ketahanan dan interpretabilitas agar hasilnya dapat dipahami oleh pengguna \citep{wang2025}. Teknik explainable artificial intelligence seperti SHAP dan LIME dapat digunakan untuk membantu menjelaskan kontribusi fitur terhadap prediksi model \citep{ribeiro2016,lundberg2017,salih2025}.

Berdasarkan uraian tersebut, penelitian ini berfokus pada deteksi potensi \textit{fraud} atau \textit{upcoding} pada klaim JKN/BPJS dalam sistem INA-CBG. Penelitian ini tidak diposisikan sebagai sistem penetapan \textit{fraud} final, melainkan sebagai pendekatan analitis untuk mengidentifikasi indikasi risiko dan membantu prioritisasi verifikasi klaim. Celah yang menjadi perhatian penelitian ini adalah kebutuhan untuk membangun pipeline \textit{claim-centric} yang menempatkan satu klaim sebagai unit analisis utama, membentuk pseudo-label ketika label \textit{fraud} aktual tidak tersedia, membandingkan beberapa model machine learning, melakukan \textit{threshold tuning}, serta menyediakan interpretabilitas global dan lokal. Dengan pendekatan tersebut, penelitian ini diharapkan dapat menghasilkan dasar analitik yang lebih terstruktur, terukur, dan dapat dijelaskan untuk membantu proses triase klaim berisiko.
